...
//改変箇所
//アクターの指定が必要だったので先頭を引きずり出した。
    DataManager.TEmakeInfoItem = function(item) {
        var actor = $gameParty.leader();
        var TEname = actor.TEname(item);
        if(!TEname) return;
        ...
        var r = '\\C['+color[3]+']';
        var text = '';
        for(var j = 1; j < $dataSystem.equipTypes.length; j++){
            var baseId = actor.TEbaseitem(item, j);
            if(!baseId) continue;
            ...
            item._afterInfos.push(r + TEname + ' ' + j);

        if (baseitem.params) {
            for (var i=0;i<8;i++) {
                var value = baseitem.params[i];
                if (value !== 0){
                    var ud = value > 0 ? g : r;
                    var sym = value > 0 ? '+' : '';
                    item._afterInfos.push(s + TextManager.param(i) + ud + sym + value );
                }
            }
        }
        if (baseitem.traits) {
            for (var i=0,max=baseitem.traits.length;i<max;i++) {
                var trait = baseitem.traits[i];
                ...
                if(text) item._afterInfos.push(text);
            }
        }
        }
    };
